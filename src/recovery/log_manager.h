...
public:
    ...
    LogRecordType get_log_record_type() const { return log_record_type_; }
...
public:
    ...
    RID get_insert_rid() const { return rid_; }
    Tuple get_insert_tuple() const { return tuple_; }
...
public:
    ...
    RID get_delete_rid() const { return rid_; }
    Tuple get_delete_tuple() const { return tuple_; }
...
public:
    ...
    UpdateLogRecord(lsn_t lsn, lsn_t prev_lsn, txn_id_t txn_id, RID rid,
                   const Tuple& old_tuple, const Tuple& new_tuple)
        : LogRecord(lsn, prev_lsn, txn_id, LogRecordType::UPDATE),
          rid_(rid), old_tuple_(old_tuple), new_tuple_(new_tuple) {}

    RID get_update_rid() const { return rid_; }
    Tuple get_old_tuple() const { return old_tuple_; }
    Tuple get_new_tuple() const { return new_tuple_; }
...
public:
    explicit LogManager(std::string log_name, size_t log_buffer_size = LOG_BUFFER_SIZE)
        : next_lsn_(0), persistent_lsn_(INVALID_LSN),
          ...
          flush_buffer_size_(0), log_name_(std::move(log_name)),
          enable_logging_(false), flush_thread_(nullptr), flush_thread_on_(false) {}
    ...
    bool is_enabled() const { return enable_logging_; }

    lsn_t get_next_lsn() const { return next_lsn_; }

    lsn_t get_persistent_lsn() const { return persistent_lsn_; }
