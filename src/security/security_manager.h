...
struct SessionInfo {
    ...
    SessionInfo(const std::string& id, const std::string& user, const std::string& ip)
        : session_id(id), username(user), client_ip(ip),
...
struct RLSPolicy {
    ...
    RLSPolicy(const std::string& name, const std::string& table,
              PermissionType perm, const std::string& condition)
...
public:
    ...
    std::vector<AuditEvent> query_audit_log(const std::string& username = "",
...
public:
    ...
    // Key management
    std::string generate_key(const std::string& key_id, EncryptionAlgorithm algorithm = EncryptionAlgorithm::AES_256_GCM);
...
public:
    ...
    AuditLogger* get_audit_logger() { return audit_logger_.get(); }
    EncryptionManager* get_encryption_manager() { return encryption_manager_.get(); }
