...
public:
    ...
    virtual std::vector<std::pair<uint64_t, double>> search(
...
public:
    ...
    VectorIndexType get_type() const override { return VectorIndexType::FLAT; }
...
private:
    struct HNSWNode {
        ...
        HNSWNode(uint64_t id, const std::vector<double>& vec)
            : id(id), vector(vec) {}
    };
...
public:
    ...
    void build_index() override;
    size_t memory_usage() const override;
    VectorIndexType get_type() const override { return VectorIndexType::HNSW; }

private:
    uint32_t get_random_level() const;
    double compute_distance(const std::vector<double>& a, const std::vector<double>& b) const;
    std::vector<std::pair<uint64_t, double>> search_layer(
        const std::vector<double>& query,
        const std::vector<uint64_t>& entry_points,
        uint32_t num_closest,
        uint32_t level
    ) const;
};
...
public:
    ...
    static std::vector<double> subtract_vectors(const std::vector<double>& a, const std::vector<double>& b);
