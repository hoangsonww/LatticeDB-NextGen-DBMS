...
public:
    ...
    bool get_value(const KeyType& key, std::vector<ValueType>* result, Transaction* transaction = nullptr);
...
private:
    static constexpr int LEAF_PAGE_SIZE = 4;
    static constexpr int INTERNAL_PAGE_SIZE = 5;
    ...
    template<typename N>
    N* reinterpret_cast_page(Page* page);

    template<typename N>
    void unpin_page(N* page, bool is_dirty);
    ...
    void insert_into_parent(BPlusTreePage* old_node, const KeyType& key,
