...
public:
    ...
    bool create_table(const std::string& table_name, const Schema& schema, Transaction* txn = nullptr);
    ...
    BufferPoolManager* get_buffer_pool_manager() { return buffer_pool_manager_.get(); }
    Catalog* get_catalog() { return catalog_.get(); }
    LockManager* get_lock_manager() { return lock_manager_.get(); }
    LogManager* get_log_manager() { return log_manager_.get(); }
    StorageEngine* get_storage_engine() { return storage_engine_.get(); }
    TransactionContext* get_transaction_context() { return transaction_context_.get(); }

    void enable_logging(bool enable) { enable_logging_ = enable; }
    void flush_logs() { if (log_manager_) log_manager_->flush(); }
    void checkpoint();
