...
public:
    Column(std::string name, ValueType type, uint32_t length = 0, bool nullable = true)
        : name_(std::move(name)), type_(type), length_(length), nullable_(nullable), has_default_(false) {}
...
public:
    ...
    uint32_t column_count() const { return static_cast<uint32_t>(columns_.size()); }

    const Column& get_column(uint32_t idx) const;

    uint32_t get_column_index(const std::string& name) const;

    uint32_t get_tuple_size() const { return tuple_size_; }

    bool has_variable_length_columns() const { return has_variable_length_columns_; }
    ...
    static std::shared_ptr<Schema> copy_schema(const Schema* from, const std::vector<uint32_t>& attrs);
