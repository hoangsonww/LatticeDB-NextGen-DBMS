...
struct StreamEvent {
    ...
    StreamEvent(const Tuple& tuple, const std::string& partition = "")
        : timestamp(std::chrono::system_clock::now()), data(tuple),
          sequence_number(0), partition_key(partition) {}
};
...
struct WindowConfig {
    ...
    static WindowConfig sliding(std::chrono::milliseconds size, std::chrono::milliseconds slide, const std::string& partition = "") {
        return {WindowType::SLIDING, size, slide, std::chrono::milliseconds(0), partition};
    }

    static WindowConfig session(std::chrono::milliseconds timeout, const std::string& partition = "") {
        return {WindowType::SESSION, std::chrono::milliseconds(0), std::chrono::milliseconds(0), timeout, partition};
    }
};
...
public:
    Window(std::chrono::system_clock::time_point start, std::chrono::system_clock::time_point end, const std::string& partition = "")
    ...
    void mark_complete() { is_complete_ = true; }

    const std::vector<StreamEvent>& get_events() const { return events_; }
    ...
    const std::string& get_partition_key() const { return partition_key_; }
...
public:
    AggregateOperator(AggregateType type, const std::string& column,
...
public:
    FilterOperator(std::unique_ptr<Expression> pred,
                   std::shared_ptr<Schema> schema,
                   std::function<void(const Tuple&)> callback);
...
public:
    JoinOperator(const std::string& join_col,
...
private:
    std::vector<std::unique_ptr<Window>> create_windows_for_event(const StreamEvent& event, const std::string& partition);
...
public:
    ...
    uint64_t get_windows_triggered() const { return windows_triggered_.load(); }
...
public:
    ...
    static bool create_stream_table(
    ...
    static std::unique_ptr<WindowManager> parse_window_clause(
        const std::string& window_spec,
        std::shared_ptr<StreamOperator> operator_
    );

    // Materialized view with streaming updates
    static bool create_streaming_materialized_view(
