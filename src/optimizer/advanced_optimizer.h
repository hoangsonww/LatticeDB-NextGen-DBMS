...
public:
    ...
    CostEstimate estimate_table_scan(const TableMetadata& table, const Expression* predicate = nullptr);
...
private:
    ...
    std::unique_ptr<PlanNode> dp_join_enumeration(const std::vector<std::unique_ptr<PlanNode>>& relations);
...
public:
    ...
    ParallelScanPlanNode(table_oid_t oid, const std::string& name,
                         std::shared_ptr<Schema> schema, uint32_t workers)
        : PlanNode(PlanNodeType::TABLE_SCAN), table_oid(oid),
          table_name(name), num_workers(workers) {
...
public:
    ...
    ParallelHashJoinPlanNode(std::unique_ptr<Expression> pred, uint32_t workers,
        ...
        : PlanNode(PlanNodeType::HASH_JOIN), join_predicate(std::move(pred)),
          num_workers(workers), hash_table_size_hint(0) {
...
public:
    ...
    WindowFunctionPlanNode(WindowFunctionType type,
                           ...
                           std::vector<OrderByClause> order,
                           std::unique_ptr<PlanNode> child)
        : PlanNode(PlanNodeType::AGGREGATE), function_type(type),
          partition_by(std::move(partition)), order_by(std::move(order)) {
        children.push_back(std::move(child));
    }
...
public:
    ...
    CTEPlanNode(const std::string& name, std::unique_ptr<PlanNode> definition, bool mat = true)
        : PlanNode(PlanNodeType::PROJECTION), cte_name(name),
          cte_definition(std::move(definition)), materialized(mat) {}
