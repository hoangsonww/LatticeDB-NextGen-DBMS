...
public:
    ...
    uint32_t get_num_pages() const { return next_page_id_; }

    bool has_free_pages() const;

    void set_flush_log(bool flush_log) { flush_log_ = flush_log; }

    uint32_t get_num_flushes() const { return num_flushes_; }

    uint32_t get_num_writes() const { return num_writes_; }
