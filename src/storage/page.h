...
public:
    ...
    inline uint32_t get_page_id() const { return page_id_; }

    inline void set_page_id(uint32_t page_id) { page_id_ = page_id; }
    ...
    inline bool is_pinned() const { return pin_count_ > 0; }

    inline bool is_dirty() const { return is_dirty_; }

    inline void set_dirty(bool dirty) { is_dirty_ = dirty; }

    inline bool is_deleted() const { return is_deleted_; }

    inline void set_deleted(bool deleted) { is_deleted_ = deleted; }
...
protected:
    template<typename T>
    inline T read_at(size_t offset) const {
        return *reinterpret_cast<const T*>(data_ + offset);
    }

    template<typename T>
    inline void write_at(size_t offset, const T& value) {
