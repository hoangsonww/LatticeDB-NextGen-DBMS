...
export default function Controls() {
  ...
  return (
    <div className={`card transition-all duration-300 ${
      ...
      {/* Header */}
      <div className={`flex items-center gap-3 px-4 py-2 border-b transition-colors duration-300 ${
      ...
      <div className="p-3 grid grid-cols-1 xl:grid-cols-3 gap-4">
        ...
        <div className="space-y-2">
          ...
          <input
            ...
            placeholder="e.g. 42"
            value={tx ?? ""}
            onChange={e => setTx(e.target.value ? parseInt(e.target.value, 10) : undefined)}
          />
          <p className={`text-xs ${darkMode ? 'text-gray-500' : 'text-gray-400'}`}>
            Query historical data at transaction <code className={`px-1 py-0.5 rounded text-xs ${
        ...
        <div className="space-y-2">
          ...
          <div className={`p-3 rounded-lg border transition-colors duration-300 ${
            ...
            <div className="mt-1 space-y-1">
              <code className={`block text-xs ${darkMode ? 'text-orange-400' : 'text-orange-600'}`}>
                VECTOR_DISTANCE(a, b)
              </code>
              <code className={`block text-xs ${darkMode ? 'text-orange-400' : 'text-orange-600'}`}>
          ...
          <p className={`text-xs ${darkMode ? 'text-gray-500' : 'text-gray-400'}`}>
            {mockMode ? 'Mock data includes vector columns' : 'Requires vector-enabled columns'}
          </p>
        </div>
      </div>
    </div>
