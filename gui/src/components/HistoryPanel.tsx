...
import { Clock, RotateCcw, CheckCircle, XCircle, Star, Copy, Trash2 } from "lucide-react"
import { useState } from "react"

export default function HistoryPanel() {
  const { history, setSql, darkMode, addToFavorites, favorites } = useStore(s => ({
    history: s.history,
    setSql: s.setSql,
    darkMode: s.darkMode,
      ...
      <div className="max-h-48 overflow-auto">
        {history.length === 0 ? (
          <div className={`p-6 text-center ${darkMode ? 'text-gray-500' : 'text-gray-400'}`}>
            <Clock size={32} className={`mx-auto mb-2 ${darkMode ? 'text-gray-600' : 'text-gray-300'}`} />
            <div className="text-sm">No history yet</div>
            <div className="text-xs mt-1">Run some queries to see them here</div>
          </div>
        ) : (
          <div className="p-2">
            {history.map((h, i) => (
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-shrink-0 mt-0.5">
                    {h.result?.ok ? (
                      ...
                      <Clock size={14} className={darkMode ? 'text-gray-500' : 'text-gray-400'} />
                    )}
                  </div>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className={`text-sm font-mono leading-relaxed ${
                      ...
                      <pre className="whitespace-pre-wrap break-words">{h.sql}</pre>
                    </div>
