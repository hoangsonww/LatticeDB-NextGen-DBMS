...
import { Table2, CheckCircle, XCircle, Clock, Download, Copy } from "lucide-react"

function cell(v: any, darkMode: boolean) {
  if (v === null || v === undefined)
    return <span className={`italic ${darkMode ? 'text-gray-500' : 'text-gray-400'}`}>NULL</span>
  if (Array.isArray(v))
    return <span className={`font-mono text-xs ${darkMode ? 'text-blue-400' : 'text-blue-600'}`}>[{v.join(", ")}]</span>
  if (typeof v === "string")
    return <span className={darkMode ? 'text-green-400' : 'text-green-600'}>'{v}'</span>
  if (typeof v === "number")
    return <span className={`font-mono ${darkMode ? 'text-yellow-400' : 'text-orange-600'}`}>{v}</span>
  return <span className={darkMode ? 'text-gray-300' : 'text-gray-700'}>{String(v)}</span>
}
    ...
    <div className={`card transition-all duration-300 ${
      ...
      <div className={`flex items-center justify-between px-4 py-2 border-b transition-colors duration-300 ${
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className={`font-medium text-sm ${darkMode ? 'text-gray-200' : 'text-gray-900'}`}>
              Query Results
            </h3>
            {result && (
            ...
            {executing && (
              <div className="flex items-center gap-2 text-xs">
                ...
                <span className={darkMode ? 'text-gray-400' : 'text-gray-500'}>Executing query...</span>
              </div>
            )}
          </div>
        </div>
        ...
        {executing ? (
          ...
          <div className="flex items-center justify-center py-8">
            <div className={`text-center ${darkMode ? 'text-gray-500' : 'text-gray-400'}`}>
              <Table2 size={32} className={`mx-auto mb-2 ${darkMode ? 'text-gray-600' : 'text-gray-300'}`} />
          ...
          <div className="p-4">
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                  ...
                  <thead>
                    <tr>
                      {result.headers.map((h, i) => (
                        <th key={i} className={`text-left font-semibold py-3 px-3 border-b-2 sticky top-0 transition-colors duration-300 ${
                          darkMode
                ...
                <tbody>
                  {rows.map((r, i) => (
                    <tr key={i} className={`transition-colors duration-200 hover:${
                      darkMode ? 'bg-gray-800/50' : 'bg-gray-50'
                    } ${i % 2 === 1 ? (darkMode ? 'bg-gray-900/50' : 'bg-gray-25') : ''}`}>
                      {r.map((c, j) => (
                        <td key={j} className={`py-2 px-3 align-top border-b transition-colors duration-300 ${
