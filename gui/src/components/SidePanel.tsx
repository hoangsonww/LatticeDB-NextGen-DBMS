...
import {
  Database, SignalHigh, SignalLow, Table, Key, Hash, Calendar,
  FileText, Star, StarOff, ChevronDown, ChevronRight, Copy,
  Play, BookOpen, Zap, BarChart3, Settings, Wifi, WifiOff
} from "lucide-react"

export default function SidePanel() {
  const {
    tables, favorites, mockMode, toggleMockMode, setSql, darkMode,
    addToFavorites, removeFromFavorites
  } = useStore()
  const [ok, setOk] = useState(false)
  const [expandedTables, setExpandedTables] = useState<Record<string, boolean>>({})
  const [activeTab, setActiveTab] = useState<'schema' | 'templates' | 'favorites'>('schema')
  ...
  const getTypeIcon = (type: string) => {
    if (type.includes('INTEGER') || type.includes('BIGINT') || type.includes('DECIMAL')) return Hash
  ...
  const getTypeColor = (type: string) => {
    if (type.includes('INTEGER') || type.includes('BIGINT') || type.includes('DECIMAL')) return 'text-blue-600'
  ...
  return (
    <div className="w-full flex flex-col gap-4 h-fit overflow-hidden">
      {/* Connection Status */}
      <div className={`card p-4 border-l-4 transition-all duration-300 ${
        darkMode
      ...
      {/* Tab Navigation */}
      <div className={`card p-0 overflow-hidden transition-colors duration-300 ${
          ...
          {activeTab === 'schema' && (
            <div className="p-4 space-y-3">
              {tables.map(table => {
                ...
                return (
                  <div key={table.name} className={`border rounded-lg overflow-hidden transition-all duration-200 hover:shadow-md ${
                    darkMode
                    ...
                    >
                      ...
                      <Table size={16} className={darkMode ? 'text-gray-400' : 'text-gray-600'} />
                    ...
                    {isExpanded && (
                      <div className={`border-t transition-colors duration-300 ${
                        ...
                        {table.columns.map(column => {
                          ...
                          return (
                            <div key={column.name} className={`flex items-center gap-2 p-2 text-sm transition-colors duration-200 ${
                              darkMode
                                ? 'hover:bg-gray-700/40'
                                : 'hover:bg-gray-50'
                            }`}>
                              <TypeIcon size={14} className={getTypeColor(column.type)} />
                              <span className={`font-mono ${darkMode ? 'text-gray-300' : 'text-gray-900'}`}>
                                {column.name}
                              </span>
                              {column.primary && <Key size={12} className="text-amber-500" title="Primary Key" />}
                              <span className={`text-xs ${getTypeColor(column.type)} ml-auto`}>
                                {column.type}
                              </span>
                              {!column.nullable && (
                ...
                <div key={category.category}>
                  ...
                  <div className="space-y-2">
                    {category.queries.map(query => (
                      <div key={query.name} className={`border rounded-lg p-3 transition-all duration-200 hover:shadow-md ${
                        darkMode
            ...
            <div className="p-4 space-y-3">
              {favorites.length === 0 ? (
                <div className={`text-center py-8 ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>
                  <Star size={48} className={`mx-auto mb-3 ${darkMode ? 'text-gray-600' : 'text-gray-300'}`} />
                  <div className="text-sm">No favorite queries yet</div>
                  <div className="text-xs mt-1">Star queries to save them here</div>
                </div>
              ) : (
                favorites.map((query, index) => (
                  <div key={index} className={`border rounded-lg p-3 transition-all duration-200 hover:shadow-md ${
                    ...
                    <div className="flex items-start gap-2">
                      <div className="flex-1">
                        <div className={`text-sm font-mono mb-2 line-clamp-3 ${
      ...
      <div className={`card p-4 transition-all duration-300 ${
        ...
        <div className={`text-sm font-semibold mb-3 ${darkMode ? 'text-gray-200' : 'text-gray-900'}`}>
          Database Stats
        </div>
        <div className="grid grid-cols-2 gap-3 text-xs">
          <div className={`rounded-lg p-3 text-center transition-all duration-200 hover:scale-105 ${
            darkMode ? 'bg-gray-900' : 'bg-white'
          }`}>
          ...
          <div className={`rounded-lg p-3 text-center transition-all duration-200 hover:scale-105 ${
            ...
            <div className="font-semibold text-lg text-green-500 mb-1">{tables.length}</div>
            <div className={darkMode ? 'text-gray-400' : 'text-gray-600'}>Tables</div>
          </div>
        </div>
      </div>
    </div>
